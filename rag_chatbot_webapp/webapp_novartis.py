...
docs = loader.load()
faiss_index = FAISS.from_documents(docs, OpenAIEmbeddings(model='text-embedding-ada-002'))

def generate_full_llm_query_faiss(query, prompt_template, limit_input_tokens=4096):
    top_results = faiss_index.similarity_search(query, k= num_chunks_of_text)
    llm_full_query = ''
    ...
    for _ in range(num_of_chunks):
        ...
        context_chunks_as_str = '\n###\n'.join([str(elem.page_content) for elem in top_results])
        ...
        if num_tokens <= limit_input_tokens:
          ...
          top_results[0].page_content = top_results[0].page_content[:-num_of_chars_to_cut]
          top_results[0].page_content = top_results[0].page_content + chunk_appendix
          print(top_results[0].page_content)
          context_chunks_as_str = '\n###\n'.join([str(elem.page_content) for elem in top_results])
          llm_full_query = prompt_template.format(context=context_chunks_as_str, question=query)
          num_tokens = len(encoding.encode(llm_full_query))
          return llm_full_query, num_tokens
...
def send_message():
    ...
    llm_full_query, context_chunks = generate_full_llm_query_faiss(query, prompt_template, limit_input_tokens=limit_input_tokens)
